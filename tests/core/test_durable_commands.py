...
from pybus.exceptions import DurableJobsNotConfiguredError
...
class RecordingDurableStore:
    def __init__(self) -> None:
        self.records: dict[str, DurableJobRecord] = {}
        self.scheduled: list[DurableJobDraft] = []
        self.claims: list[DurableJobClaim] = []
        self.published: list[DurableJobClaim] = []
        self.indeterminate: list[DurableJobClaim] = []
    ...
    def schedule(self, draft: DurableJobDraft) -> DurableJobRecord:
        ...
        if existing is not None:
            return existing
        record = DurableJobRecord.from_draft(draft)
    ...
    def claim(self, *, worker_id, now, lease_expires_at):
        record = next(
            (
                ...
                if value.state == DurableJobState.PENDING
            ),
            None,
        )
        if record is None:
        ...
        claimed = replace(
            record,
            state=DurableJobState.CLAIMED,
            generation=record.generation + 1,
            lease_owner=worker_id,
            lease_expires_at=lease_expires_at,
        )
        self.records[record.id] = claimed
        claim = DurableJobClaim.from_record(claimed)
    ...
    def mark_published(self, claim, *, now, reconciliation_due_at):
        ...
        self.records[claim.id] = replace(
            current,
            state=DurableJobState.PUBLISHED,
...
class RecordingRecurringStore(RecordingDurableStore):
    ...
    def schedule_recurring(self, draft):
        record = DurableJobRecord.from_draft(draft.first_occurrence)
        series_id = f"series-{len(self.series) + 1}"
        record = replace(record, series_id=series_id, occurrence_number=1)
        self.records[record.id] = record
        series = JobSeriesRecord(
            id=series_id,
            state=JobSeriesState.ACTIVE,
...
def test_schedule_command_is_concise_and_performs_no_transport_io() -> None:
    ...
    bus = Pybus(transport, durable_job_store=store)

    handle = bus.schedule_command(GenerateBill(student_id=7))

    assert isinstance(handle, DurableJobHandle)
...
def test_recurrence_requires_an_explicit_store_capability() -> None:
    bus = Pybus(MemoryTransport(), durable_job_store=RecordingDurableStore())

    with pytest.raises(JobSeriesNotSupportedError):
...
def test_durable_worker_uses_configured_policy_and_accepts_an_override() -> None:
    configured = DurableJobPolicy(
        lease_duration=timedelta(minutes=2),
        reconciliation_delay=timedelta(minutes=20),
    )
    override = DurableJobPolicy(
        lease_duration=timedelta(minutes=3),
        reconciliation_delay=timedelta(minutes=30),
    )
    bus = BusConfiguration(
        transport_factory=MemoryTransport,
        durable_job_store_factory=RecordingDurableStore,
        durable_job_policy=configured,
    ).create()

    worker = bus.create_durable_job_worker()
    overridden_worker = bus.create_durable_job_worker(policy=override)
...
def test_empty_durable_worker_paces_database_polling() -> None:
    ...
    bus.create_durable_job_worker(idle_delay=0.01).run(max_iterations=1)
...
def test_schedule_command_requires_explicit_durable_configuration() -> None:
    ...
    with pytest.raises(DurableJobsNotConfiguredError):
        bus.schedule_command(GenerateBill(student_id=7))
    with pytest.raises(DurableJobsNotConfiguredError):
        bus.create_durable_job_worker()
...
def test_durable_worker_publishes_a_generation_fenced_delivery_copy() -> None:
    ...
    worker = bus.create_durable_job_worker(error_delay=0)
    ...
    assert envelope.message_kind == "command"
...
def test_unknown_publish_acknowledgement_is_recorded_and_aborts_worker() -> None:
    ...
    store = RecordingDurableStore()
    bus = Pybus(FailingTransport(), durable_job_store=store)
...
def test_post_publication_state_failures_abort_as_indeterminate(failure) -> None:
    ...
    bus = Pybus(PublishBehavior(), durable_job_store=FailingStore())
...
def test_durable_envelope_requires_a_store_on_the_consuming_worker() -> None:
    ...
    transport = MemoryTransport()
    publishing_bus = Pybus(transport, durable_job_store=RecordingDurableStore())
    publishing_bus.schedule_command(GenerateBill(student_id=101))
    publishing_bus.create_durable_job_worker(error_delay=0).run(max_iterations=1)
...
def test_durable_configuration_does_not_change_ordinary_multi_handler_commands() -> (
    ...
    bus = Pybus(
        MemoryTransport(),
        durable_job_store=RecordingDurableStore(),
...
def test_current_generation_is_admitted_and_terminal_outcome_is_persisted() -> None:
    ...
    store = RecordingDurableStore()
    bus = Pybus(MemoryTransport(), durable_job_store=store, handler_targets=[handle])
...
def test_retry_settlement_is_checkpointed_before_the_delivery_outcome() -> None:
    ...
    store = RecordingDurableStore()
    bus = Pybus(MemoryTransport(), durable_job_store=store, handler_targets=[fail])
